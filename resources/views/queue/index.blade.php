...
<style>
    ...
    .waiting-column .column-header { border-bottom-color: #f59e0b; color: #92400e; }
    .calling-column .column-header { border-bottom-color: #dc2626; color: #991b1b; }
    .treatment-column .column-header { border-bottom-color: #0ea5e9; color: #0369a1; }
    .payment-column .column-header { border-bottom-color: #8b5cf6; color: #6d28d9; }
    .completed-column .column-header { border-bottom-color: #10b981; color: #166534; }
...
<div class="container-fluid">
    ...
    <div class="page-header">
        <div class="d-flex justify-content-between align-items-center">
            <div>
                <h2><i class="bi bi-list-ol me-2"></i>ระบบคิว</h2>
                <p>{{ now()->locale('th')->isoFormat('วันdddd D MMMM Y') }}</p>
            </div>
            <div class="d-none d-md-block">
                <a href="{{ url('/queue/display') }}" target="_blank" class="btn btn-light btn-sm">
                    <i class="bi bi-tv me-1"></i>จอ TV
                </a>
            </div>
        </div>
    </div>
                ...
                <div class="column-content">
                    @forelse($waitingItems as $item)
                        <div class="queue-card">
                            <div class="patient-name" style="font-size: 0.9rem; padding: 0.3rem 0;">{{ $item['name'] }}</div>
                            <div class="card-actions">
                                <button class="btn btn-info" onclick="callQueue('{{ $item['id'] }}', '{{ $item['name'] }}', {{ $item['number'] }})">
                                    <i class="bi bi-megaphone me-1"></i>เรียกคิว
                                </button>
                                <button class="btn btn-outline-danger" onclick="cancelAppointment('{{ $item['id'] }}', '{{ $item['name'] }}')">
                ...
                <div class="column-content">
                    @forelse($callingItems as $item)
                        <div class="queue-card calling-card">
                            <div class="patient-name" style="font-size: 1rem; font-weight: 600; padding: 0.3rem 0; color: #dc2626;">{{ $item['name'] }}</div>
                            <div class="card-actions">
                                <button class="btn btn-primary" onclick="startTreatment('{{ $item['id'] }}', '{{ $item['name'] }}')">
                                    <i class="bi bi-play-fill me-1"></i>เริ่มรักษา
                                </button>
                            </div>
                            <div class="card-actions mt-1">
                                <button class="btn btn-outline-secondary btn-sm" onclick="recallQueue('{{ $item['id'] }}', {{ $item['number'] }})">
                                    <i class="bi bi-arrow-repeat me-1"></i>เรียกซ้ำ
                                </button>
                                <button class="btn btn-outline-danger btn-sm" onclick="skipQueue('{{ $item['id'] }}', '{{ $item['name'] }}')">
                ...
                <div class="column-content">
                    @forelse($inTreatmentItems as $item)
                        <div class="queue-card">
                            ...
                            <div class="card-actions">
                                <button class="btn btn-warning" onclick="finishTreatment('{{ $item['id'] }}', '{{ $item['name'] }}', '{{ $item['patient_id'] }}', '{{ $item['started_at'] ?? now() }}')">
                                    <i class="bi bi-stop-fill me-1"></i>เสร็จสิ้น
                                </button>
                                <button class="btn btn-outline-danger" onclick="cancelTreatmentAppointment('{{ $item['id'] }}', '{{ $item['name'] }}')">
                    ...
                    @forelse($awaitingPaymentItems as $item)
                        <div class="queue-card payment-card">
                            ...
                            <div class="card-actions">
                                ...
                                <button class="btn btn-outline-danger" onclick="cancelPayment('{{ $item['id'] }}', '{{ $item['name'] }}')">
                                    <i class="bi bi-arrow-counterclockwise"></i>
                                </button>
                            </div>
                        </div>
                ...
                <div class="column-content">
                    @forelse($completedItems as $item)
                        <div class="queue-card completed-card">
                            <div class="patient-name" style="font-size: 0.9rem; padding: 0.3rem 0;">{{ $item['name'] }}</div>
                            <div class="card-actions">
                                <button class="btn btn-primary btn-sm" onclick="printReceipt('{{ $item['id'] }}')">
                                    <i class="bi bi-printer me-1"></i>ปริ้นใบเสร็จ
                                </button>
                                <button class="btn btn-outline-warning btn-sm" onclick="editCompletedPayment('{{ $item['id'] }}', '{{ $item['name'] }}', '{{ $item['patient_id'] }}')">
