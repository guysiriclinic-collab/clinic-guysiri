            ...
            <div class="row">
                <div class="col-12 col-lg-8">
                    ...
                    <div class="timeline">
                        @php
                            $appointments = $patient->appointments()
                                ->with(['treatments.service', 'treatments.pt', 'pt'])
                        ...
                        <div class="timeline-item">
                            <div class="timeline-icon {{ $appointment->status == 'completed' ? 'success' : ($appointment->status == 'scheduled' ? '' : 'warning') }}"></div>
                                ...
                                <div class="d-flex justify-content-between align-items-start">
                                    <div>
                                        <h6 class="fw-bold mb-1">
                                            ...
                                            @else
                                                {{ $appointment->purpose ?? 'นัดหมาย' }}
                                            @endif
                                        </h6>
                                        <small style="color: #64748b;">
                                            @php $apptDateItem = \Carbon\Carbon::parse($appointment->appointment_date); @endphp
                                            <i class="bi bi-calendar3 me-1"></i>
                                            {{ $apptDateItem->locale('th')->isoFormat('D MMM') }} {{ $apptDateItem->year + 543 }}
                                            {{ \Carbon\Carbon::parse($appointment->appointment_time)->format('H:i') }} น.
                                        </small>
                                        @if($treatment && $treatment->pt)
                                        <br><small style="color: #64748b;">
                                            <i class="bi bi-person me-1"></i>PT: {{ $treatment->pt->name ?? $treatment->pt->username }}
                                        </small>
                                        @elseif($appointment->pt)
                                        <br><small style="color: #64748b;">
                                            <i class="bi bi-person me-1"></i>PT: {{ $appointment->pt->name ?? $appointment->pt->username }}
                                        </small>
                                        @endif
                                        @if($treatment && $treatment->duration_minutes)
                                        <br><small style="color: #64748b;">
                                    ...
                                    <div class="text-end">
                                        <span class="badge mb-2" style="background: {{ $appointment->status == 'completed' ? '#dcfce7' : ($appointment->status == 'scheduled' ? '#dbeafe' : '#fef3c7') }}; color: {{ $appointment->status == 'completed' ? '#166534' : ($appointment->status == 'scheduled' ? '#1e40af' : '#92400e') }};">
                                            @if($appointment->status == 'completed')
                                                เสร็จสิ้น
                                            @elseif($appointment->status == 'scheduled' || $appointment->status == 'pending')
                                                รอรักษา
                                            @else
                                                {{ $appointment->status }}
                                            @endif
                                        </span>
                                        @if($appointment->status == 'completed' && $treatment)
                                        <br>
                                        <button class="btn btn-outline-primary btn-sm mt-1" onclick="viewTreatmentDetail('{{ $appointment->id }}')">
                                            <i class="bi bi-eye me-1"></i>ดูรายละเอียด
                                        </button>
                                        @endif
                                    </div>
                                </div>
                                @if($treatment && $treatment->treatment_notes)
                                <div class="bg-light rounded p-2 mt-2">
                                    <small><i class="bi bi-chat-text me-1"></i>{{ Str::limit($treatment->treatment_notes, 100) }}</small>
                                </div>
                                @elseif($appointment->notes)
                                <div class="bg-light rounded p-2 mt-2">
                                    <small><i class="bi bi-chat-text me-1"></i>{{ $appointment->notes }}</small>
                                </div>
                                @endif
                            </div>
                        </div>
                    ...
                    <button class="btn btn-outline-secondary btn-sm w-100 mt-2">
                        <i class="bi bi-arrow-down me-1"></i>ดูเพิ่มเติม
                    </button>
                    @endif
                </div>
